                  ...
                  @foreach ($users as $user)
                    @if (isset($user) && !empty($user))
                      <tr>
                        ...
                        <td>
                          {{-- {{ user Created }} --}}
                          @if (isset($user->user_created) && !empty($user->user_created) && $user->user_created != "0000-00-00 00:00:00")
                        ...
                        <td>
                          ...
                          @if (isset($user->user_id) && !empty($user->user_id) && Auth::id() != null && !empty(Auth::id()) && $user->user_id != Auth::id())
                            ...
                            {!!Form::open(['action' => ['UsersController@destroy', $user->user_id, "false"], 'method' => 'POST', 'class' =>'visible-sm-inline-block visible-md-inline-block visible-xs-inline-block visible-lg-inline-block visible-xl-inline-block deleteUserForm'])!!}
  ...
  <script type="text/javascript">
    $(document).ready( function(){
      ...
      $(".deleteUserForm").on("submit", function(){
        ...
        var confirm = window.confirm("Are you sure you wish to permantly delete the user " + user_name + " with the email address " + user_email + "?");
