...
exports.default = function (paper, researcher) {
  ...
  */
	sentences.forEach(function (sentence) {
		var matchesInSentence = topicForms.keyphraseForms.map(function (keywordForms) {
			return (0, _matchTextWithArray2.default)(sentence, keywordForms, locale);
		});
		var hasAllKeywords = matchesInSentence.every(function (keywordForm) {
			return keywordForm.count > 0;
		});

		if (hasAllKeywords) {
			var counts = matchesInSentence.map(function (match) {
				return match.count;
			});
			var foundWords = (0, _lodashEs.flattenDeep)(matchesInSentence.map(function (match) {
